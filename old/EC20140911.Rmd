...
Data
-------------------------
* Data received from Anne Hoos in September 2014 as 4 sas files and save to C:\Bryan\EPA\Data\Sparrow\EastCoast\Predictions20140911
...
* There are only 22 lakes in MRB1 with differences in calculated TN and 28 lakes with differences in flow.
* These can be combined with the lake locations and written to a shapefile to send to Ann and Rich for checking.

```{r makeShp, include=FALSE, echo=FALSE, cache=FALSE}
  #Get the Spatial Data
    load(url('https://raw.github.com/willbmisled/ECsparrow/master/data/lakesSpatialData.rda'))
  #Get data for lakes to check
    con <- odbcConnectAccess('C:/Bryan/EPA/Data/Sparrow/EastCoast/EC2014.mdb')
      Data<-sqlQuery(con, "SELECT tempCheckLakes.* FROM tempCheckLakes")
    close(con)
      Data$WBRCHCODE<-paste('0',Data$WBRCHCODE,sep='')  #replace zeros that Excel removed
  #merge data with lake spdf
    test<-merge(lakes,Data,by='WBRCHCODE',all=FALSE)
  #output shapefile
    writeOGR(test,"data","lakesToCheck",driver="ESRI Shapefile",)


SELECT tempCheckLakes.* FROM tempCheckLakes;


setwd('./data/)
con<-odbcConnectExcel('./compare.xls',readOnly = TRUE)
sqlTables(con)
N<-sqlFetch(con,'compare',rownames=F,colnames=F)
