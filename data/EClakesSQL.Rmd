...
```{r data, include=FALSE, echo=FALSE, cache=TRUE}
  ...
  #function to read the SAS file and write to the SQLite database
      #' Name: name of the table to create
      #' File: name of the SAS file to read
      #' Conn=EClakes: connection file; default = EClakes.sqlite    
    getSas<-function(Name,File,Conn=EClakes){
      ##release some RAM
        gc() #garbage collection: cleans up temp files
      #read the SAS file
        assign(Name,read.sas7bdat(File))
      #add to EClakes.sqlite
        dbWriteTable(conn=Conn,name=Name,value=get(Name),row.names=FALSE,overwrite=TRUE)
    }

#read the data and write the tables
  getSas("R2002m1","C:/Bryan/EPA/Data/Sparrow/EastCoast/Predictions20140911/mrb1_massbalancereservoirs_2002.sas7bdat")
  
  getSas("R2002m2","C:/Bryan/EPA/Data/Sparrow/EastCoast/Predictions20140923/mrb2_massbalancereservoirs_2002.sas7bdat")
  
  getSas("R2020m1","C:/Bryan/EPA/Data/Sparrow/EastCoast/Predictions20140911/mrb1_massbalancereservoirs_2020.sas7bdat")
  
  getSas("R2020m2","C:/Bryan/EPA/Data/Sparrow/EastCoast/Predictions20140923/mrb2_massbalancereservoirs_2020.sas7bdat")

#Reachcode field named "WBRCHCODE" for MRB1 and "WBRchCd" for MRB2
#Rename field in MRB1 tables for consistency
  a<-dbReadTable(EClakes, "R2002m2")
    names(a)[2]<-"WBRCHCODE" 
      dbWriteTable(conn=EClakes,name='R2002m2',value=a,row.names=FALSE,overwrite=TRUE)
  a<-dbReadTable(EClakes, "R2020m2")
    names(a)[2]<-"WBRCHCODE" 
      dbWriteTable(conn=EClakes,name='R2020m2',value=a,row.names=FALSE,overwrite=TRUE)

#dbListTables(EClakes) # List the tables in the database
```
