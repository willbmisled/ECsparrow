...
```{r setup, include=FALSE, echo=FALSE, cache=TRUE}
  ...
  #function to install (if needed) and load R packages by list
    libs<-c('RSQLite','sas7bdat') #list of packages to load
...
#set source for EClakes.sqlite
  EClakes<-dbConnect(SQLite(), dbname='EClakes.sqlite')
```

```{r R2002m1, include=FALSE, echo=FALSE, cache=TRUE}
      #release some RAM
        gc() #garbage collection: cleans up temp files
      #connection info
        EClakes<-dbConnect(SQLite(), dbname='EClakes.sqlite')
      #read the SAS file
        R2002m1<-read.sas7bdat("C:/Bryan/EPA/Data/Sparrow/EastCoast/Predictions20140911/mrb1_massbalancereservoirs_2002.sas7bdat")
      #add to EClakes.sqlite
        dbWriteTable(conn=EClakes,name='R2002m1',value=R2002m1,row.names=FALSE,overwrite=TRUE)
```

```{r R2020m1, include=FALSE, echo=FALSE, cache=TRUE}
      #release some RAM
        gc() #garbage collection: cleans up temp files
      #connection info
        EClakes<-dbConnect(SQLite(), dbname='EClakes.sqlite')
      #read the SAS file
        R2020m1<-read.sas7bdat("C:/Bryan/EPA/Data/Sparrow/EastCoast/Predictions20140911/mrb1_massbalancereservoirs_2020.sas7bdat")
      #add to EClakes.sqlite
        dbWriteTable(conn=EClakes,name='R2020m1',value=R2020m1,row.names=FALSE,overwrite=TRUE)
```

```{r R2002m2, include=FALSE, echo=FALSE, cache=TRUE}
      #release some RAM
        gc() #garbage collection: cleans up temp files
      #connection info
        EClakes<-dbConnect(SQLite(), dbname='EClakes.sqlite')
      #read the SAS file
        R2002m2<-read.sas7bdat("C:/Bryan/EPA/Data/Sparrow/EastCoast/Predictions20141003/mrb2_massbalancereservoirs_2002.sas7bdat")
      #rename "MOFLOWU" to "TOT_CFS" to match MRB1
        names(R2002m2)[which(names(R2002m2)=='MAFLOWU')]<-'TOT_CFS'
      #rename "WBRchCd" to "WBRCHCODE" to match MRB1
        names(R2002m2)[which(names(R2002m2)=='WBRchCd')]<-'WBRCHCODE'
      #add to EClakes.sqlite
        dbWriteTable(conn=EClakes,name='R2002m2',value=R2002m2,row.names=FALSE,overwrite=TRUE)
```

```{r R2020m2, include=FALSE, echo=FALSE, cache=TRUE}
      #release some RAM
        gc() #garbage collection: cleans up temp files
      #connection info
        EClakes<-dbConnect(SQLite(), dbname='EClakes.sqlite')
      #read the SAS file
        R2020m2<-read.sas7bdat("C:/Bryan/EPA/Data/Sparrow/EastCoast/Predictions20141003/mrb2_massbalancereservoirs_2020.sas7bdat")
      #rename "MOFLOWU" to "TOT_CFS" to match MRB1
        names(R2020m2)[which(names(R2020m2)=='MAFLOWU')]<-'TOT_CFS'
      #rename "WBRchCd" to "WBRCHCODE" to match MRB1
        names(R2020m2)[which(names(R2020m2)=='WBRchCd')]<-'WBRCHCODE'
      #add to EClakes.sqlite
        dbWriteTable(conn=EClakes,name='R2020m2',value=R2020m2,row.names=FALSE,overwrite=TRUE)
```
